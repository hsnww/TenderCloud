...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div class="form-group m-3">
                ...
                <input type="radio" name="role" value="" {{ !$user->roles->count() ? 'checked' : '' }} onclick="toggleAssociationType()"> No role<br>
                @foreach($roles as $role)
                    <input type="radio" name="role" value="{{ $role->name }}" {{ $user->hasRole($role->name) ? 'checked' : '' }} onclick="toggleAssociationType()"> {{ ucfirst($role->name) }}<br>
                @endforeach
            </div>

            <div class="form-group m-3" id="association-type" style="display: none;">
                <label>Association Type</label><br>
                <input type="radio" name="association_type" value="company" {{ $user->companies()->exists() ? 'checked' : '' }} onclick="selectRoleAndAssociation('company')"> Company<br>
                <input type="radio" name="association_type" value="vendor" {{ $user->vendors()->exists() ? 'checked' : '' }} onclick="selectRoleAndAssociation('vendor')"> Vendor<br>
            </div>
            ...
            <div class="form-group m-3" id="company-select" style="display: none;">
                ...
                <select name="company_id" id="company_id" class="form-control">
                    ...
                    @foreach($companies as $company)
                        <option value="{{ $company->id }}" {{ $user->companies->contains($company->id) ? 'selected' : '' }}>
                            {{ $company->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group m-3" id="vendor-select" style="display: none;">
                ...
                <select name="vendor_id" id="vendor_id" class="form-control">
                    ...
                    @foreach($vendors as $vendor)
                        <option value="{{ $vendor->id }}" {{ $user->vendors->contains($vendor->id) ? 'selected' : '' }}>
                            {{ $vendor->name }}
                        </option>
                    @endforeach
                </select>
    ...
    <script>
        ...
        // Initialize the display based on the selected role
